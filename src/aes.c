#include "aes.h"

static const uint8_t sbox[256] = {
    
};

void sub_bytes(uint8_t *bloc) {
    for (int i = 0; i < 16; i++) {
        bloc[i] = sbox[bloc[i]];
    }
}
void shift_rows(uint8_t *bloc) {
    uint8_t temp;

    // Ligne 1 : decalage de 1 vers la gauche
    temp = bloc[1];
    bloc[1] = bloc[5];
    bloc[5] = bloc[9];
    bloc[9] = bloc[13];
    bloc[13] = temp;

    // Ligne 2 : decalage de 2 vers la gauche (echange des paires)
    temp = bloc[2];
    bloc[2] = bloc[10];
    bloc[10] = temp;
    temp = bloc[6];
    bloc[6] = bloc[14];
    bloc[14] = temp;

    // Ligne 3 : decalage de 3 vers la gauche (= 1 vers la droite)
    temp = bloc[15];
    bloc[15] = bloc[11];
    bloc[11] = bloc[7];
    bloc[7] = bloc[3];
    bloc[3] = temp;
}
static uint8_t xtime(uint8_t x) {
    uint8_t resultat = x << 1;
    if (x & 0x80) {
        resultat ^= 0x1b;
    }
    return resultat;
}

void mix_columns(uint8_t *bloc) {
    for (int col = 0; col < 4; col++) {
        int i = col * 4;

        uint8_t a0 = bloc[i];
        uint8_t a1 = bloc[i + 1];
        uint8_t a2 = bloc[i + 2];
        uint8_t a3 = bloc[i + 3];

        bloc[i]     = xtime(a0) ^ (xtime(a1) ^ a1) ^ a2 ^ a3;
        bloc[i + 1] = a0 ^ xtime(a1) ^ (xtime(a2) ^ a2) ^ a3;
        bloc[i + 2] = a0 ^ a1 ^ xtime(a2) ^ (xtime(a3) ^ a3);
        bloc[i + 3] = (xtime(a0) ^ a0) ^ a1 ^ a2 ^ xtime(a3);
    }
}
void add_round_key(uint8_t *bloc, const uint8_t *round_key) {
    for (int i = 0; i < 16; i++) {
        bloc[i] ^= round_key[i];
    }
}
static const uint8_t rcon[11] = {
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};
void key_expansion(const uint8_t *cle, uint8_t *round_keys) {
    // Les 16 premiers octets = la cle originale, copiee telle quelle
    for (int i = 0; i < 16; i++) {
        round_keys[i] = cle[i];
    }

    uint8_t temp[4];
    int bytes_generes = 16;
    int rcon_index = 1;

    while (bytes_generes < 176) {
        // Recupere les 4 derniers octets generes
        for (int j = 0; j < 4; j++) {
            temp[j] = round_keys[bytes_generes - 4 + j];
        }

        // Cas special : tous les 16 octets (4 mots)
        if (bytes_generes % 16 == 0) {
            // RotWord : rotation d'1 position vers la gauche
            uint8_t t = temp[0];
            temp[0] = temp[1];
            temp[1] = temp[2];
            temp[2] = temp[3];
            temp[3] = t;

            // SubWord : passage par la S-box
            for (int j = 0; j < 4; j++) {
                temp[j] = sbox[temp[j]];
            }

            // XOR avec Rcon, uniquement sur le premier octet
            temp[0] ^= rcon[rcon_index];
            rcon_index++;
        }

        // Nouveau mot = mot d'il y a 16 octets XOR temp
        for (int j = 0; j < 4; j++) {
            round_keys[bytes_generes+j] = round_keys[bytes_generes - 16 + j] ^ temp[j];
        }
        bytes_generes += 4;
    }
}
void aes128_encrypt(const uint8_t *bloc_entree, const uint8_t *cle, uint8_t *bloc_sortie) {
    uint8_t etat[16];
    uint8_t round_keys[176];

    // Copie le bloc d'entree dans "etat", pour ne jamais modifier bloc_entree
    for (int i = 0; i < 16; i++) {
        etat[i] = bloc_entree[i];
    }

    // Genere les 11 sous-cles a partir de la cle principale
    key_expansion(cle, round_keys);

    // Tour initial : juste un AddRoundKey
    add_round_key(etat, round_keys);

    // 9 tours complets
    for (int tour = 1; tour <= 9; tour++) {
        sub_bytes(etat);
        shift_rows(etat);
        mix_columns(etat);
        add_round_key(etat, round_keys + tour * 16);
    }

    // Tour final : sans MixColumns
    sub_bytes(etat);
    shift_rows(etat);
    add_round_key(etat, round_keys + 10 * 16);

    // Copie le resultat dans bloc_sortie
    for (int i = 0; i < 16; i++) {
        bloc_sortie[i] = etat[i];
    }
}
static const uint8_t inv_sbox[256] = {
    
};

void inv_sub_bytes(uint8_t *bloc) {
    for (int i = 0; i < 16; i++) {
        bloc[i] = inv_sbox[bloc[i]];
    }
}
void inv_shift_rows(uint8_t *bloc) {
    uint8_t temp;

    // Ligne 1 : decalage de 1 vers la droite (inverse de shift_rows)
    temp = bloc[13];
    bloc[13] = bloc[9];
    bloc[9] = bloc[5];
    bloc[5] = bloc[1];
    bloc[1] = temp;

    // Ligne 2 : decalage de 2 (identique dans les deux sens, une paire d'echanges)
    temp = bloc[2];
    bloc[2] = bloc[10];
    bloc[10] = temp;
    temp = bloc[6];
    bloc[6] = bloc[14];
    bloc[14] = temp;

    // Ligne 3 : decalage de 3 vers la droite (= 1 vers la gauche)
    temp = bloc[3];
    bloc[3] = bloc[7];
    bloc[7] = bloc[11];
    bloc[11] = bloc[15];
    bloc[15] = temp;
}
static uint8_t mul(uint8_t x, uint8_t y) {
    uint8_t resultat = 0;
    while (y) {
        if (y & 1) {
            resultat ^= x;
        }
        x = xtime(x);
        y >>= 1;
    }
    return resultat;
}
void inv_mix_columns(uint8_t *bloc) {
    for (int col = 0; col < 4; col++) {
        int i = col * 4;

        uint8_t a0 = bloc[i];
        uint8_t a1 = bloc[i + 1];
        uint8_t a2 = bloc[i + 2];
        uint8_t a3 = bloc[i + 3];

        bloc[i]     = mul(a0, 0x0e) ^ mul(a1, 0x0b) ^ mul(a2, 0x0d) ^ mul(a3, 0x09);
        bloc[i + 1] = mul(a0, 0x09) ^ mul(a1, 0x0e) ^ mul(a2, 0x0b) ^ mul(a3, 0x0d);
        bloc[i + 2] = mul(a0, 0x0d) ^ mul(a1, 0x09) ^ mul(a2, 0x0e) ^ mul(a3, 0x0b);
        bloc[i + 3] = mul(a0, 0x0b) ^ mul(a1, 0x0d) ^ mul(a2, 0x09) ^ mul(a3, 0x0e);
    }
}
void aes128_decrypt(const uint8_t *bloc_entree, const uint8_t *cle, uint8_t *bloc_sortie) {
    uint8_t etat[16];
    uint8_t round_keys[176];

    for (int i = 0; i < 16; i++) {
        etat[i] = bloc_entree[i];
    }

    key_expansion(cle, round_keys);

    // Tour initial : AddRoundKey avec la DERNIERE sous-cle
    add_round_key(etat, round_keys + 10 * 16);

    // 9 tours, a l'envers (de 9 a 1)
    for (int tour = 9; tour >= 1; tour--) {
        inv_shift_rows(etat);
        inv_sub_bytes(etat);
        add_round_key(etat, round_keys + tour * 16);
        inv_mix_columns(etat);
    }

    // Tour final : sans InvMixColumns
    inv_shift_rows(etat);
    inv_sub_bytes(etat);
    add_round_key(etat, round_keys);   // round_keys[0], la cle d'origine

    for (int i = 0; i < 16; i++) {
        bloc_sortie[i] = etat[i];
    }
}